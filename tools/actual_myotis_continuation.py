...
def aligned_errors(result):
    ...
    return (
        rms_position_error(aligned_microphones, true_microphones),
        rms_position_error(aligned_sources, true_sources),
    )
preview_mic, preview_src = aligned_errors(preview)
final_mic, final_src = aligned_errors(final)
print(
    f"scale={scale:g} budget={preview_budget} preview_post={preview.negative_log_posterior:.9f} "
    f"preview_mic={preview_mic:.9f} preview_src={preview_src:.9f} "
    f"preview_success={preview.success} final_post={final.negative_log_posterior:.9f} "
    f"final_mic={final_mic:.9f} final_src={final_src:.9f} "
    f"final_nrms={final.normalized_data_rms:.9f} success={final.success}",
    flush=True,
)
